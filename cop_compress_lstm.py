...
class Alignment_Compression_Base(nn.Module):
    def __init__(self,
        ...
        self.Resnet2d_lstm_dim_converter_self = nn.Sequential(
            nn.Linear(lstm_hidden, lstm_hidden//2),
            nn.Linear(lstm_hidden//2, lstm_hidden//4),
            nn.Linear(lstm_hidden//4, 64),
            nn.ReLU(inplace=True)
        )
        self.Resnet2d_lstm_dim_converter_feat2d = nn.Sequential(
            nn.Linear(lstm_hidden, lstm_hidden//2),
            nn.Linear(lstm_hidden//2, lstm_hidden//4),
            nn.Linear(lstm_hidden//4, 64),
            nn.ReLU(inplace=True)
        )
        self.Resnet2d_feat2d_dim_converter_self = nn.Sequential(
            nn.Linear(feat_2d_dim, 16),
            nn.Linear(16, 32),
            nn.Linear(32, 64),
            nn.ReLU(inplace=True)
        )
        self.Resnet2d_feat2d_dim_converter_lstm = nn.Sequential(
            nn.Linear(feat_2d_dim, 16),
            nn.Linear(16, 32),
            nn.Linear(32, 64),
            nn.ReLU(inplace=True)
        )
        self.Resnet2d_lstm = ResNet2d(Block_configs)
        self.Resnet2d_feat2d = ResNet2d(Block_configs)
    ...
    def forward(self, x_1d, x_2d):
        ...
        # print(x_1d.shape)
        x1d_to_1d = self.Resnet2d_lstm_dim_converter_self(x_1d)
        x1d_to_2d = self.Resnet2d_lstm_dim_converter_feat2d(x_1d)
        x2d_to_2d = self.Resnet2d_feat2d_dim_converter_self(x_2d)
        x2d_to_1d = self.Resnet2d_feat2d_dim_converter_lstm(x_2d)
        # x = torch.cat((x, x_2d), 1)

        x_1d = (x1d_to_1d + x2d_to_1d).permute(0, 3, 1, 2)
        x_2d = (x2d_to_2d + x1d_to_2d).permute(0, 3, 1, 2)
        x_2d = self.Resnet2d_feat2d(x_2d)
        x_1d = self.Resnet2d_lstm(x_1d)
        x = x_2d + x_1d
        x = self.dropout(x)
        x = self.conv_fin(x)
        x = self.activation(x)
