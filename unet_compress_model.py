...
class Unet_Compression_Base(nn.Module):
    def __init__(self,
        ...
        super(Unet_Compression_Base, self).__init__()

        self.Compresser1 = md_cnn.DownConv1d([alignment_features,
                                              alignment_features*2,
                                              alignment_features*2],
                                             kernels=[5, 3])
        self.Compresser2 = md_cnn.DownConv1d([alignment_features*2,
                                              alignment_features*4,
                                              alignment_features*4],
                                             kernels=[5, 3])

        self.compresser2lstm_conv = nn.Conv1d(in_channels=alignment_features*4,
        ...
        self.Upconv1 = md_cnn.UpConv1d(channels=[lstm_out,
                                                 ...
                                                 lstm_out//2],
                                       kernels=[5, 3],
                                       shortcut_channels=alignment_features*2)
        self.Upconv2 = md_cnn.UpConv1d(channels=[lstm_out//2,
                                                 lstm_out//4,
                                                 lstm_out//4],
                                       kernels=[5, 3],
                                       shortcut_channels=alignment_features)
...
RB2d_configs = [
    ("conv", ResBlock_config([5, 3, 5], [
     alignment_features*4, 128, 98, 64], "relu")),
...
def train_compression_model():
    ...
    # set up models
    model = Unet_Compression_Base(alignment_features,
                                  lstm_input_size,
                                  lstm_hidden,
                                  lstm_layer,
                                  RB2d_configs)
