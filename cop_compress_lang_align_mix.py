...
class Alignment_Compression_Base(nn.Module):
    def __init__(self,
        ...
        # compressed 1d alignment channel
        ...
        self.compresser2lstm_conv = nn.Conv1d(in_channels=compresser2_config.channel[-1],
                                              out_channels=ali_input_size,
                                              kernel_size=2)

        self.RNN_ali = BiLSTM(input_size=ali_input_size,
                              hidden_size=lstm_hidden_ali,
                              dropout=0.0,
                              layer=lstm_layers,
                              shortcut=True,
                              carryon_info=False)

        lstm_out_ali = lstm_hidden_ali*2
        self.Upconv1 = md_cnn.UpConv1d(channels=[lstm_out_ali,
                                                 lstm_out_ali//2,
                                                 lstm_out_ali//2],
                                       kernels=[5, 3],
                                       shortcut_channels=compresser2_config.channel[0])
        self.Upconv2 = md_cnn.UpConv1d(channels=[lstm_out_ali//2,
                                                 lstm_out_ali//4,
                                                 lstm_out_ali//4],
                                       kernels=[5, 3],
        ...
        self.RNN_lan = BiLSTM(input_size=Bert_output_size,
                              hidden_size=lstm_hidden_lan,
                              dropout=0.0,
                              layer=lstm_layers,
                              shortcut=True,
                              carryon_info=False)
        lstm_out_lan = 2*lstm_hidden_lan

        # 2d coupler
        # alignment
        self.Pipe2d_ali_dim_converter_self = nn.Sequential(
            nn.Linear(lstm_hidden_ali, lstm_hidden_ali//2),
            nn.Linear(lstm_hidden_ali//2, lstm_hidden_ali//4),
            nn.Linear(lstm_hidden_ali//4, 64),
            nn.ReLU(inplace=True)
        )
        self.Pipe2d_ali_dim_converter_feat2d = nn.Sequential(
            nn.Linear(lstm_hidden_ali, lstm_hidden_ali//2),
            nn.Linear(lstm_hidden_ali//2, lstm_hidden_ali//4),
            nn.Linear(lstm_hidden_ali//4, 64),
            nn.ReLU(inplace=True)
        )
        self.Pipe2d_ali_dim_converter_lang = nn.Sequential(
            nn.Linear(lstm_hidden_ali, lstm_hidden_ali//2),
            nn.Linear(lstm_hidden_ali//2, lstm_hidden_ali//4),
            nn.Linear(lstm_hidden_ali//4, 64),
            nn.ReLU(inplace=True)
        ...
        self.Pipe2d_feat2d_dim_converter_ali = nn.Sequential(
            nn.Linear(feat_2d_dim, 16),
            nn.Linear(16, 32),
            nn.Linear(32, 64),
            nn.ReLU(inplace=True)
        )
        self.Pipe2d_feat2d_dim_converter_lang = nn.Sequential(
            nn.Linear(feat_2d_dim, 16),
            nn.Linear(16, 32),
            nn.Linear(32, 64),
            nn.ReLU(inplace=True)
        ...
        # language
        self.Pipe2d_lang_dim_converter_self = nn.Sequential(
            nn.Linear(lstm_out_lan*2, lstm_out_lan),
            nn.Linear(lstm_out_lan, lstm_out_lan//4),
            nn.Linear(lstm_out_lan//4, lstm_out_lan//8),
            nn.Linear(lstm_out_lan//8, 64),
            nn.ReLU(inplace=True)
        )
        self.Pipe2d_lang_dim_converter_ali = nn.Sequential(
            nn.Linear(lstm_out_lan*2, lstm_out_lan),
            nn.Linear(lstm_out_lan, lstm_out_lan//4),
            nn.Linear(lstm_out_lan//4, lstm_out_lan//8),
            nn.Linear(lstm_out_lan//8, 64),
        ...
        self.Pipe2d_lang_dim_converter_feat2d = nn.Sequential(
            nn.Linear(lstm_out_lan*2, lstm_out_lan),
            nn.Linear(lstm_out_lan, lstm_out_lan//4),
            nn.Linear(lstm_out_lan//4, lstm_out_lan//8),
            nn.Linear(lstm_out_lan//8, 64),
        ...
        self.Pipe2d_align = Pipe2d_Compression_Base(Block_configs_precompr,
                                                    Block_configs_1,
                                                    Block_configs_2)
        self.Pipe2d_feat2d = Pipe2d_Compression_Base(Block_configs_precompr,
                                                     Block_configs_1,
                                                     Block_configs_2)
        self.Pipe2d_lang = Pipe2d_Compression_Base(Block_configs_precompr,
                                                   Block_configs_1,
                                                   Block_configs_2)
    ...
    def forward(self, x_1d_ali, x_1d_lan, x_2d):
        # 1d ali
        shortcut1 = x_1d_ali
        x_1d_ali = self.Compresser1(x_1d_ali)
        shortcut2 = x_1d_ali
        x_1d_ali = self.Compresser2(x_1d_ali)
        x_1d_ali = self.compresser2lstm_conv(x_1d_ali)
        x_1d_ali = x_1d_ali.permute(0, 2, 1)
        x_1d_ali = self.RNN_ali(x_1d_ali)
        x_1d_ali = x_1d_ali.permute(0, 2, 1)

        x_1d_ali = self.Upconv1(x_1d_ali, shortcut2)
        x_1d_ali = self.Upconv2(x_1d_ali, shortcut1)

        x_1d_ali = self.to_2d(x_1d_ali)
        x_1d_ali = x_1d_ali.permute(0, 2, 3, 1)

        # 1d lang
        x_1d_lan = self.bert(x_1d_lan)[0]
        x_1d_lan = x_1d_lan[:, 1:-1, :]
        x_1d_lan = self.RNN_lan(x_1d_lan)
        x_1d_lan = x_1d_lan.permute(0, 2, 1)
        x_1d_lan = self.to_2d(x_1d_lan)
        x_1d_lan = x_1d_lan.permute(0, 2, 3, 1)

        # Dim Coupling
        x_ali_to_ali = self.Pipe2d_ali_dim_converter_self(x_1d_ali)
        x_ali_to_2d = self.Pipe2d_ali_dim_converter_feat2d(x_1d_ali)
        x_ali_to_lan = self.Pipe2d_ali_dim_converter_lang(x_1d_ali)

        x2d_to_2d = self.Pipe2d_feat2d_dim_converter_self(x_2d)
        x2d_to_ali = self.Pipe2d_feat2d_dim_converter_ali(x_2d)
        x2d_to_lan = self.Pipe2d_feat2d_dim_converter_lang(x_2d)

        x_lan_to_lan = self.Pipe2d_lang_dim_converter_self(x_1d_lan)
        x_lan_to_ali = self.Pipe2d_lang_dim_converter_ali(x_1d_lan)
        x_lan_to_2d = self.Pipe2d_lang_dim_converter_feat2d(x_1d_lan)

        # info sharing
        x_1d_ali = (x_ali_to_ali + x2d_to_ali +
                    x_lan_to_ali).permute(0, 3, 1, 2)
        x_1d_lan = (x_lan_to_lan + x_ali_to_lan +
                    x2d_to_lan).permute(0, 3, 1, 2)
        x_2d = (x2d_to_2d + x_ali_to_2d + x_lan_to_2d).permute(0, 3, 1, 2)

        # 2d piping
        x_1d_ali = self.Pipe2d_align(x_1d_ali)
        x_1d_lan = self.Pipe2d_lang(x_1d_lan)
        x_2d = self.Pipe2d_feat2d(x_2d)

        # outputing
        x = x_2d + x_1d_ali + x_1d_lan
        x = self.dropout(x)
        x = self.conv_fin(x)
        x = self.activation(x)
...
lstm_hidden_ali = alignment_features*4
lstm_hidden_lan = 196
lstm_layer = 4
