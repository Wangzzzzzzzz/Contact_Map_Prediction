...
from md_train import train_an_epoch, eval_an_epoch, split_train_val
...
class Language_Base(nn.Module):
    def __init__(self,
        ...
        self.conv2d = nn.Conv2d(RB2d_config[-1][1].channel[3], 2, 1)
...
Bert_output_dim = 768
lstm_hidden_lang = 128
lstm_layer_lang = 2

RB1d_confis_lang = [
    ("conv", ResBlock_config([3, 5, 1], [
        Bert_output_dim, 256, 256, 128], "relu")),
    ("id", ResBlock_config([3, 5, 1], [128, 128, 128, 128], "relu")),
]

RB2d_configs_lang = [
    ("conv", ResBlock_config([3, 5, 1], [
        lstm_hidden_lang*4, 256, 256, 128], "relu")),
...
alignment_features = 64
lstm_hidden_ali = 88
lstm_layer_ali = 2

RB1d_confis_ali = [
    ("id", ResBlock_config([3, 5, 1], [alignment_features,
                                       alignment_features,
                                       alignment_features,
                                       alignment_features], "relu")),
    ("id", ResBlock_config([3, 5, 1], [alignment_features,
                                       alignment_features,
                                       alignment_features,
                                       alignment_features], "relu")),
    ("id", ResBlock_config([3, 5, 1], [alignment_features,
                                       ...
                                       alignment_features], "relu")),
    ("conv", ResBlock_config([3, 5, 1], [
        alignment_features, 128, 128, 256], "relu")),
]

RB2d_configs_ali = [
    ("conv", ResBlock_config([3, 5, 1], [
        lstm_hidden_ali*4, 256, 256, 128], "relu")),
    ("id", ResBlock_config([3, 5, 1], [128, 32, 32, 128], "relu")),
    ("id", ResBlock_config([3, 5, 1], [128, 32, 32, 128], "relu")),
    ("id", ResBlock_config([3, 5, 1], [128, 32, 32, 128], "relu")),
    ("conv", ResBlock_config([3, 5, 1], [128, 32, 32, 64], "relu")),
...
def train_seq_model_along():
    # set up datasets
    train_x, train_y, eval_x, eval_y = split_train_val(
        '../seq_data/data_set.pkl', 0.9)
    ...
    sequentail_channel_model = Language_Contact(Bert_output_dim,
                                                ...
                                                lstm_layer_lang,
                                                RB2d_configs_lang)
    optimizer = optim.Adam(sequentail_channel_model.parameters(), lr=0.000005)
...
def train_align_model_along():
    ...
    alignment_channel_model = Alignment_Contact(alignment_features,
                                                ...
                                                lstm_layer_ali,
                                                RB2d_configs_ali)
    optimizer = optim.Adam(alignment_channel_model.parameters(), lr=0.000005)
    # loss_fn = CTmap_CrossEntropyLoss(weight=torch.tensor([0.1,2.5]),
    #                                  ignore_diag=6)
    loss_fn = nn.CrossEntropyLoss(weight=torch.tensor([0.20, 1.5]))
